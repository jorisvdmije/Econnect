import fetchEconnect from "./fetch-econnect.js";
import fetchWorkflow from "./fetch-workflow.js";
...
export const main = async () => {
  const jsonData = await fs.readFile(
    "/Users/jorisvdmije/Desktop/programming/webscraping/econnect/w/dist/data.json"
  );
  const data = JSON.parse(jsonData.toString());
  const groepen = data["groepen"];

  const econnectData = data["econnect_data_niet_gekoppeld"];
  //   const econnectData = [];
  let fetchedIds = [];
  const { fetchedEconnenctData, fetchedWorkflowData } = await mainAsync();

  const workflowData = []; // Assuming this is defined somewhere in your context
